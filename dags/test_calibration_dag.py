...
with DAG(dag_id="test_calibration", start_date=days_ago(1)) as dag:
    ...
    CalibrationDatesJump >> S3SecurityMasterReader >> S3IndustryMappingReader >> S3EconTransformationReader >> YahooDailyPriceReader >> S3RussellComponentReader >> S3RawQuandlDataReader >> DownloadEconomicData >> QuandlDataCleanup >> CalculateTargetReturns >> CalculateDerivedQuandlFeatures >> CalculateTaLibSTOCHRSIMultiParam >> CalculateVolatilityMultiParam >> CalculateTaLibWILLRMultiParam >> CalculateTaLibPPOMultiParam >> CalculateTaLibADXMultiParam >> ComputeBetaQuantamental >> CalculateMACD
