...
def get_X_cols():
    return ['PPO_12_26_indgrp', 'et', 'IPDCONGD', 'ebitda_to_ev_indgrp', 'revenue_yield', 'T5YFFM', 'rf', 'ret_1B', 'overnight_return', 'grossmargin', 'netmargin_indgrp', 'maxret', 'OILPRICEx', 'currentratio', 'ensemble', 'EWG_close', 'STOCHRSI_FASTK_63_15_10', 'bm', 'PPO_21_126', 'current_to_total_assets', 'ret_63B', 'revenue_growth', 'PPO_21_126_Industrials', 'ret_2B', 'PPO_21_126_Energy', 'ebitdamargin', 'debt2equity_indgrp', 'COMPAPFFx', 'ret_126B', 'log_dollar_volume', 'debt2equity', 'investments_yield', 'fcf_yield', 'volatility_126_indgrp', 'macd_diff_indgrp', 'STOCHRSI_FASTD_14_5_3', 'PPO_21_126_indgrp', 'amihud', 'VXOCLSx', 'ebitda_to_ev', 'pe1', 'macd_diff_ConsumerStaples', 'T1YFFM', 'volatility_21', 'gbm', 'netmargin', 'ret_10B', 'pe_indgrp', 'divyield', 'WILLR_14', 'HWI', 'CUSR0000SAC', 'divyield_indgrp', 'ret_63B_indgrp', 'payoutratio', 'EWJ_volume', 'PPO_12_26', 'PPO_3_14', 'retearn_to_total_assets', 'std_turn', 'log_avg_dollar_volume', 'ret_252B', 'EXUSUKx', 'rel_to_high', 'ADX_14_MA_3', 'macd_diff', 'capex_yield', 'bm_indgrp', 'PPO_21_126_InformationTechnology', 'gp2assets']
...
def cut_insample_data(df, end_year, end_month):
...
def get_algos():
    algos = {
        ...
        'et': ExtraTreesRegressor(n_estimators=500, min_samples_leaf=0.001, random_state=seed, n_jobs=17,
                                  max_features=17, max_depth=10),
        'gbm': GradientBoostingRegressor(min_samples_leaf=0.001, random_state=seed, n_estimators=400,
...
def run_for_date(df, output_dir, end_year, end_month, run_all=True, run_parallel=False, dump_file=False):
    #df = pd.read_hdf(data_file)
    folds, X,y,n,p, monthly_final_date = cut_insample_data(df, end_year, end_month)
