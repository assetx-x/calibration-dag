# Shamelessly copied from bx-python repository (https://bitbucket.org/james_taylor/bx-python/src/)
# file: bx-python / lib / bx / intervals / intersection.pyx, master branch, sha2 = da37e3a
...
cdef class IntervalNode:
    ...
    def __cinit__(IntervalNode self, int start, int end, object interval):
        ...
        self.priority = ceil(nlog * log(-1.0/(1.0 * np.random.rand() - 1)))
        self.start    = start
        self.end      = end
