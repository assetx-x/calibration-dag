...
class marketTimeline(object):
    ...
    def get_non_trading_days(start, end, timezone_corr='utc'):
        ...
        return base_dates[(base_dates>=pd.Timestamp(start)) & (base_dates<=pd.Timestamp(end))]
    ...
    def get_trading_days(start, end, as_long=False):
        ...
        dates =  pd.date_range(start=start,
                               ...
                               freq=trading_day)
        dates = dates if not as_long else dates.tz_localize("UTC").normalize().asi8.astype(long)
        return dates
    ...
    def get_trading_day_using_offset(cob, offset_in_business_days):
        trading_days = marketTimeline.get_trading_days(FIRST_POSSIBLE_TRADING_DATE, LAST_POSSIBLE_TRADING_DATE,
                                                       as_long=True)
        cob_long = cob.tz_localize(None).normalize().asm8.astype(long)
        date_loc = trading_days.searchsorted(cob_long, "left")
        return pd.Timestamp(trading_days[date_loc+offset_in_business_days-(trading_days[date_loc]!=cob_long)*(offset_in_business_days>0)].astype("M8[ns]"))
    ...
    @staticmethod
    def marketIsOpened(dt):
        # Checks if market is opened at current dt;
        # It is opened when 9:30 pm EST <= dt < 14:30 pm EST and
        # current day is trading day.
        # Requires: dt to be in UTC time.

        if not marketTimeline.isTradingDay(dt):
            return False
        else:
            marketOpeningTime = datetime.time(hour=9, minute=30, tzinfo=pytz.timezone('EST'))
            marketClosingTime = datetime.time(hour=16, tzinfo=pytz.timezone('EST'))
            utcTimeBasedOnDt = datetime.time(hour=dt.hour, minute=dt.minute, tzinfo=pytz.timezone('UTC'))
            return (marketOpeningTime <= utcTimeBasedOnDt < marketClosingTime)

#if __name__=="__main__":
#    hh = marketTimeline()
#    print hh.get_trading_days(datetime.datetime(2015,1,1), datetime.datetime(2020,1,3)).tolist()
