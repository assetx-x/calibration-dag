...
class FilterMonthlyDatesFullPopulation(DataReaderClass):
    ...
    def do_step_action(self, **kwargs):
        ...
        if self.mode in ["bme", "bms"]:
            ...
            filtered_data = df[
                (df["date"] >= self.start_date) & (df["date"] <= max_end_date)
            ]
...
class FilterMonthlyDatesFullPopulationWeekly(FilterMonthlyDatesFullPopulation):
    ...
    def __init__(self, start_date, end_date, monthly_mode, weekly_mode):
        ...
        FilterMonthlyDatesFullPopulation.__init__(
            self, start_date, end_date, monthly_mode
        )
    ...
    def do_step_action(self, **kwargs):
        ...
        return {
            "monthly_merged_data": self.data,
            "weekly_merged_data": self.weekly_data,
        }
...
class CreateMonthlyDataSingleNames(DataReaderClass):
    ...
    def do_step_action(self, **kwargs):
        ...
        singlename_tickers = list(
            security_master.dropna(subset=["IndustryGroup"])["dcm_security_id"].unique()
        )
        res = monthly_merged_data[
            monthly_merged_data["ticker"].isin(singlename_tickers)
...
class CreateMonthlyDataSingleNamesWeekly(CreateMonthlyDataSingleNames):
    ...
    def do_step_action(self, **kwargs):
        ...
        self.weekly_data = weekly_merged_data[
            weekly_merged_data["ticker"].isin(singlename_tickers)
        ].reset_index(drop=True)
        return {
            'monthly_merged_data_single_names': self.data,
            'weekly_merged_data_single_names': self.weekly_data,
        }
...
FilterMonthlyDatesFullPopulationWeekly_params = {
    'params': FMDP_params,
    'class': FilterMonthlyDatesFullPopulationWeekly,
    'start_date': RUN_DATE,
    'provided_data': {
        'monthly_merged_data': construct_destination_path('filter_dates_single_name'),
        'weekly_merged_data': construct_destination_path('filter_dates_single_name'),
    },
    'required_data': {
        'merged_data': construct_required_path('merge_step', 'merged_data')
    },
}
...
CreateMonthlyDataSingleNamesWeekly_params = {
    'params': {},
    'class': CreateMonthlyDataSingleNamesWeekly,
    'start_date': RUN_DATE,
    'provided_data': {
        'monthly_merged_data_single_names': construct_destination_path(
            'filter_dates_single_name'
        ),
        'weekly_merged_data_single_names': construct_destination_path(
            'filter_dates_single_name'
        ),
    },
    'required_data': {
        'monthly_merged_data': construct_required_path(
            'filter_dates_single_name', 'monthly_merged_data'
        ),
        'weekly_merged_data': construct_required_path(
            'filter_dates_single_name', 'weekly_merged_data'
        ),
