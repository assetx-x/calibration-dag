...
class ComputeBetaQuantamental(DataReaderClass):
    ...
    def __init__(
        ...
        self.benchmark_names = benchmark_names or [
            8554
        ]  # ["SPY"] # This is overridden for now
        ...
        if use_robust:
            raise ValueError(
                "Currently robust regrssion is not supported. use_robust must be False."
            )
    ...
    def _regression_for_all_dates(self, indep_var, depend_vars):
        ...
        for dt in self.dates_to_compute:
            ...
            y_mat = depend_vars.loc[start_date:dt].dropna(
                thresh=self.required_records, axis=1
            )
            result_df = (
                get_beta(
                    X,
                    ...
                    use_robust_method=self.use_robust,
            ...
            betas.loc[dt, result_df.columns] = result_df.loc[
                result_df.index[0], result_df.columns
            ]
        return betas
    ...
    def do_step_action(self, **kwargs):
        ...
        pivot_data = (
            daily_prices[["date", "ticker", self.price_column]]
            .pivot_table(
                index="date", values=self.price_column, columns="ticker", dropna=False
            )
            .sort_index()
        )
        ...
        if self.use_robust:
            betas = {
                "{0}_beta".format(i): (
                    self._regression_for_all_dates(benchmark_df.loc[:, i], return_data)
                )
                for i in benchmark_df.columns
            }
        else:
            denom = return_data.rolling(
                min_periods=self.beta_lookback, window=self.beta_lookback, center=False
            ).var()
            betas = {
                "{0}_beta".format(i): (
                    return_data.rolling(window=self.beta_lookback).cov(
                        other=benchmark_df.loc[:, i], pairwise=self.beta_lookback
                    )
                    / denom
                )
                for i in benchmark_df.columns
        ...
        betas = {
            "{0}".format(benchmark): betas[benchmark].shift() for benchmark in betas
        }
...
class CalculateMACD(DataReaderClass):
    ...
    def do_step_action(self, **kwargs):
        ...
        indicator_func = (
            lambda x: self.technical_indicator(
                x[self.price_column].sort_index(), **self.technical_indicator_params
            )
            .shift(shift_value)
            .to_frame(name="macd")
        )
        ...
        indicator_signal["macd_centerline"] = (
            indicator_signal.groupby(["ticker"])["macd"]
            .apply(lambda x: MA(x, self.smoothing_period, type='simple'))
            .reset_index(level=0, drop=True)
        )
        indicator_signal["macd_diff"] = (
            indicator_signal["macd"] - indicator_signal["macd_centerline"]
        )
...
class CalcualteCorrelation(DataReaderClass):
    ...
    def do_step_action(self, **kwargs):
        ...
        rolling_return_data = return_data.rolling(
            self.correlation_lookback, self.correlation_lookback
        )
        ...
        correlations = {
            "{0}_correl".format(i): rolling_return_data.corr(benchmark_df.loc[:, i])
            for i in benchmark_df.columns
        }

        correlations = {
            benchmark_corr: correlations[benchmark_corr].shift()
            for benchmark_corr in correlations
        }
        rank_correlations = {
            "{0}_rank".format(i): correlations[i].rank(axis=1) for i in correlations
        }
        correlations.update(rank_correlations)
        all_correlation_data = []
        for concept in correlations:
            this_data = pd.melt(
                correlations[concept].reset_index(),
                var_name="ticker",
                id_vars="date",
                value_name=concept,
            ).set_index(["date", "ticker"])
...
class CalculateDollarVolume(DataReaderClass):
    ...
    def do_step_action(self, **kwargs):
        ...
        df_1 = pd.melt(
            avg_dollar_volumes.reset_index(),
            id_vars=['date'],
            var_name='ticker',
...
class CalculateOvernightReturn(DataReaderClass):
    ...
    def do_step_action(self, **kwargs):
        ...
        daily_prices = (
            kwargs[self.__class__.REQUIRES_FIELDS[0]]
            .sort_values(["ticker", "date"])
            .round(3)
        )
        return_func = lambda g: (
            g["open"].combine_first(g["close"].fillna(method="ffill").shift())
            / g["close"].fillna(method="ffill").shift()
            - 1.0
        ).to_frame(name="overnight_return")
        apply_func = lambda x: return_func(x.set_index("date"))
        self.data = (
            daily_prices.groupby("ticker")
            .apply(apply_func)
            .reset_index()[["date", "ticker", "overnight_return"]]
        )
        return self.data
...
class CalculatePastReturnEquity(DataReaderClass):
    ...
    def calculate_past_performance_for_group(self, g):
        ...
        for shift_amount in self.lookback_list:
            g_copy["ret_{0}B".format(str(shift_amount))] = (
                g[column].shift(day_shift)
                / g[column].shift(day_shift).shift(shift_amount)
                - 1.0
            )
        return g_copy
    ...
    def do_step_action(self, **kwargs):
        ...
        res = daily_prices.groupby("ticker").apply(
            self.calculate_past_performance_for_group
        )
        added_cols = list(map(lambda x: "ret_{0}B".format(str(x)), self.lookback_list))
        self.data = res.drop(["ticker"], axis=1).reset_index()[
            ["date", "ticker"] + added_cols
        ]
...
class CalculateTaLibSTOCH(DataReaderClass):
    ...
    def do_step_action(self, **kwargs):
        ...
        # TODO: Check whether indiscriminantly rounding is a good idea
        high = (
            daily_prices.pivot_table(
                index='date', columns='ticker', values='high', dropna=False
            )
            .sort_index()
        ...
        low = (
            daily_prices.pivot_table(
                index='date', columns='ticker', values='low', dropna=False
            )
            .sort_index()
            .fillna(method='ffill', limit=3)
            .shift(shift_value)
        ...
        close = (
            daily_prices.pivot_table(
                index='date', columns='ticker', values='close', dropna=False
            )
            .sort_index()
            .fillna(method='ffill', limit=3)
            .shift(shift_value)
        ...
        indicator_generator = (
            self.technical_indicator(
                high[c], low[c], close[c], **self.technical_indicator_params
            )
            for c in high.columns
        )
        slow_fast_indicators = zip(*[k for k in indicator_generator])
        slow_fast_stoch_kd = list(
            map(lambda x: pd.concat(x, axis=1, keys=high.columns), slow_fast_indicators)
        )
    ...
    def do_step_action(self, **kwargs):
        ...
        indicator_func = (
            lambda x: self.technical_indicator(
                ...
                **self.technical_indicator_params
            )
            .shift(shift_value)
            .to_frame(
                name="TRIX_{0}".format(self.technical_indicator_params["timeperiod"])
            )
        )
        apply_func = lambda x: indicator_func(x.set_index("date"))
        indicator_signal = (
...
class CalculateTaLibULTOSC(DataReaderClass):
    ...
    def do_step_action(self, **kwargs):
        ...
        column_name = 'ULTOSC_{0}_{1}_{2}'.format(
            self.technical_indicator_params["timeperiod1"],
        ...
        indicator_signal = (
            indicator_signal.stack().reset_index().rename(columns={0: column_name})
        )

        indicator_signal[ma_column_name] = (
            indicator_signal.groupby(["ticker"])[column_name]
            .apply(lambda x: MA(x, self.smoothing_period, type='simple'))
            .reset_index(level=0, drop=True)
        )
        self.data = indicator_signal  # .set_index(["date","ticker"])
...
CalculateMACD_params = {
    ...
    'start_date': RUN_DATE,
    'provided_data': {
        'macd_indicator_data': construct_destination_path('derived_simple')
    },
    'required_data': {
        'daily_price_data': construct_required_path('data_pull', 'daily_price_data')
    },
...
CalculateDollarVolume_params = {
    'params': {"lookback_periods": 21},
    'class': CalculateDollarVolume,
    'start_date': RUN_DATE,
    'provided_data': {
        'dollar_volume_data': construct_destination_path('derived_simple')
    },
    'required_data': {
        'daily_price_data': construct_required_path('data_pull', 'daily_price_data')
    },
...
CalculateOvernightReturn_params = {
    'params': {},
    'class': CalculateOvernightReturn,
    'start_date': RUN_DATE,
    'provided_data': {
        'overnight_return_data': construct_destination_path('derived_simple')
    },
    'required_data': {
        'daily_price_data': construct_required_path('data_pull', 'daily_price_data')
    },
...
CalculatePastReturnEquity_params = {
    'params': {"column": "close", "lookback_list": [1, 2, 5, 10, 21, 63, 126, 252]},
    'class': CalculatePastReturnEquity,
    'start_date': RUN_DATE,
...
CalculateTaLibSTOCH_params = {
    'params': {'technical_indicator_params': configs_CalculateTaLibSTOCH},
    'class': CalculateTaLibSTOCH,
    'start_date': RUN_DATE,
    'provided_data': {
        'talib_stoch_indicator_data': construct_destination_path('derived_simple')
    },
    'required_data': {
        'daily_price_data': construct_required_path('data_pull', 'daily_price_data')
    },
...
CalculateTaLibSTOCHF_params = {
    'params': {'technical_indicator_params': CalculateTaLibSTOCHF_configs},
    'class': CalculateTaLibSTOCHF,
    'start_date': RUN_DATE,
    'provided_data': {
        'talib_stochf_indicator_data': construct_destination_path('derived_simple')
    },
    'required_data': {
        'daily_price_data': construct_required_path('data_pull', 'daily_price_data')
    },
...
CalculateTaLibTRIX_params = {
    'params': {
        'technical_indicator_params': talib_trix_indicator_data,
        'price_column': 'close',
    },
    'class': CalculateTaLibTRIX,
    'start_date': RUN_DATE,
    'provided_data': {
        'talib_trix_indicator_data': construct_destination_path('derived_simple')
    },
    'required_data': {
        'daily_price_data': construct_required_path('data_pull', 'daily_price_data')
    },
...
CalculateTaLibULTOSC_params = {
    ...
    'class': CalculateTaLibULTOSC,
    'start_date': RUN_DATE,
    'provided_data': {
        'talib_ultosc_indicator_data': construct_destination_path('derived_simple')
    },
    'required_data': {
        'daily_price_data': construct_required_path('data_pull', 'daily_price_data')
    },
