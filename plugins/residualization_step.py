...
class FactorNeutralizationForStacking(DataReaderClass):
    ...
    def do_step_action(self, **kwargs):
        ...
        assert set(r1k_models) == set(
            FILTER_MODES
        ), "FactorNeutralizationForStacking - r1k_models doesn't seem to contain all \
        expected modes. It contains- {0}".format(
            set(r1k_models)
        )
        ...
        for mode in r1k_models:
            ...
            r1k_resid_dict[mode] = neutralize_data(
                r1k_models[mode], self.factors, self.exclusion_list
            )
...
class FactorNeutralizationForStackingWeekly(FactorNeutralizationForStacking):
    ...
    def do_step_action(self, **kwargs):
        ...
        r1k_models_monthly = self._dictionary_format(
            growth=kwargs["r1k_models_growth"],
        ...
        r1k_models_sc_weekly = self._dictionary_format(
            growth=kwargs["r1k_models_sc_weekly_growth"],
            value=kwargs["r1k_models_sc_weekly_value"],
        )

        r1k_models_lc_weekly = self._dictionary_format(
            largecap_growth=kwargs["r1k_models_lc_weekly_largecap_growth"],
            largecap_value=kwargs["r1k_models_lc_weekly_largecap_value"],
        )

        assert set(r1k_models_monthly) == set(
            FILTER_MODES
        ), "FactorNeutralizationForStacking - r1k_models_monthly doesn't seem to \
        contain all expected modes. It contains- {0}".format(
            set(r1k_models_monthly)
        )
        assert set(r1k_models_sc_weekly).union(set(r1k_models_lc_weekly)) == set(
            FILTER_MODES
        ), "FactorNeutralizationForStacking - \
        r1k_models_weekly doesn't seem to contain all expected modes. \
        It contains- {0}".format(
            set(r1k_models_sc_weekly).union(r1k_models_lc_weekly)
        )
        ...
        for mode in r1k_models_monthly:
            print("Neutralizing r1k data for {0} model".format(mode))
            r1k_resid_dict_monthly[mode] = neutralize_data(
                r1k_models_monthly[mode], self.factors, self.exclusion_list
            )
            if "largecap" in mode:
                r1k_resid_lc_dict_weekly[mode] = neutralize_data(
                    r1k_models_lc_weekly[mode], self.factors, self.exclusion_list
                )
            else:
                r1k_resid_sc_dict_weekly[mode] = neutralize_data(
                    r1k_models_sc_weekly[mode], self.factors, self.exclusion_list
                )

        self.r1k_resid_models = pd.DataFrame(
            list(r1k_resid_dict_monthly.items()), columns=['Key', 0]
        ).set_index('Key')
        self.r1k_resid_sc_weekly = pd.DataFrame(
            list(r1k_resid_sc_dict_weekly.items()), columns=['Key', 0]
        ).set_index('Key')
        self.r1k_resid_lc_weekly = pd.DataFrame(
            list(r1k_resid_lc_dict_weekly.items()), columns=['Key', 0]
        ).set_index('Key')
    ...
    def _get_additional_step_results(self):
        return {
            "r1k_resid_models_growth": self.r1k_resid_models.loc['growth'][0],
            "r1k_resid_models_value": self.r1k_resid_models.loc['value'][0],
            "r1k_resid_models_largecap_value": self.r1k_resid_models.loc[
                'largecap_growth'
            ][0],
            "r1k_resid_models_largecap_growth": self.r1k_resid_models.loc[
                'largecap_value'
            ][0],
            ...
            "r1k_resid_lc_weekly_largecap_growth": self.r1k_resid_lc_weekly.loc[
                'largecap_growth'
            ][0],
            "r1k_resid_lc_weekly_largecap_value": self.r1k_resid_lc_weekly.loc[
                'largecap_value'
            ][0],
        }
...
FactorNeutralizationForStackingWeekly_params = {
    'params': params,
    "start_date": RUN_DATE,
    'class': FactorNeutralizationForStackingWeekly,
    'provided_data': {
        'r1k_resid_models': construct_destination_path('residualization'),
        'r1k_resid_sc_weekly': construct_destination_path('residualization'),
        'r1k_resid_lc_weekly': construct_destination_path('residualization'),
    },
    'required_data': {
        'r1k_models': construct_required_path('population_split', 'r1k_models'),
        'r1k_models_sc_weekly': construct_required_path(
            'population_split', 'r1k_models_sc_weekly'
        ),
        'r1k_models_lc_weekly': construct_required_path(
            'population_split', 'r1k_models_lc_weekly'
        ),
    },
}
