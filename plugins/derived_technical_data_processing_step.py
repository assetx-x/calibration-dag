    ...
    def do_step_action(self, **kwargs):
        ...
        price = (
            daily_prices.pivot_table(
                index='date', columns='ticker', values=self.price_column, dropna=False
            )
            .sort_index()
        ...
        indicator_generator = (
            self.technical_indicator(price[c], **self.technical_indicator_params)
            for c in price.columns
        )
        slow_fast_indicators = zip(*[k for k in indicator_generator])
        slow_fast_stoch_kd = list(
            map(
                lambda x: pd.concat(x, axis=1, keys=price.columns), slow_fast_indicators
            )
        )
        ...
        fastk = (
            ...
            .rename(
                columns={
                    0: 'STOCHRSI_FASTK_{0}_{1}_{2}'.format(
                        self.technical_indicator_params['timeperiod'],
        ...
        fastd = (
            ...
            .rename(
                columns={
                    0: 'STOCHRSI_FASTD_{0}_{1}_{2}'.format(
                        self.technical_indicator_params['timeperiod'],
                        self.technical_indicator_params['fastk_period'],
                        self.technical_indicator_params['fastd_period'],
                    )
...
class CalculateTaLibSTOCHRSIMultiParam(DataReaderClass):
    ...
    def do_step_action(self, **kwargs):
        ...
        for config in self.configs:
            ...
            result = list(indicator._get_additional_step_results().values())[
                0
            ].set_index(["date", "ticker"])
...
class CalculateVolatility(DataReaderClass):
    ...
    def do_step_action(self, **kwargs):
        ...
        vol_func = lambda g: (
            np.log(g[self.price_column]).diff().rolling(self.volatility_lookback).std()
        ).to_frame(name=column_name)
        apply_func = lambda x: vol_func(
            x.set_index("date").sort_index().shift(shift_value)
        )
        self.data = (
            daily_prices.groupby("ticker")
            .apply(apply_func)
            .reset_index()[["date", "ticker", column_name]]
        )
...
class CalculateTaLibPPO(DataReaderClass):
    ...
    def __init__(self, technical_indicator_params, price_column, invert_sign):
        ...
        self.invert_sign = (
            invert_sign or True
        )  # talib sign convention is the opposite of everywhere else
        # CalibrationTaskflowTask.__init__(self)
    ...
    def do_step_action(self, **kwargs):
        ...
        ).shift(shift_value).to_frame(
            name="PPO_{0}_{1}".format(
                self.technical_indicator_params["fastperiod"],
...
class CalculateTaLibADX(DataReaderClass):
    ...
    def __init__(
        self, technical_indicator, technical_indicator_params, smoothing_period
    ):
        self.data = None
...
CalculateTaLibSTOCHRSIMultiParam_PARAMS = {
    'params': {'configs': CalculateTaLibSTOCHRSIMultiParam_figs},
    'class': CalculateTaLibSTOCHRSIMultiParam,
    'start_date': RUN_DATE,
    'provided_data': {
        'talib_stochrsi_indicator_data': construct_destination_path('derived_technical')
    },
    'required_data': {
...
CalculateVolatilityMultiParam_params = {
    'params': {'configs': CalculateVolatilityMultiParam_configs},
    'class': CalculateVolatilityMultiParam,
    'start_date': RUN_DATE,
    'provided_data': {
        'volatility_data': construct_destination_path('derived_technical')
    },
    'required_data': {
        'daily_price_data': construct_required_path('data_pull', 'daily_price_data')
    },
...
CalculateTaLibWILLRMultiParam_params = {
    'params': {'configs': CalculateTaLibWILLRMultiParam_configs},
    'class': CalculateTaLibWILLRMultiParam,
    'start_date': RUN_DATE,
    'provided_data': {
        'talib_willr_indicator_data': construct_destination_path('derived_technical')
    },
    'required_data': {
        'daily_price_data': construct_required_path('data_pull', 'daily_price_data')
    },
...
CalculateTaLibPPOMultiParam_params = {
    'params': {'configs': CalculateTaLibPPOMultiParam_configs},
    'class': CalculateTaLibPPOMultiParam,
    'start_date': RUN_DATE,
    'provided_data': {
        'talib_ppo_indicator_data': construct_destination_path('derived_technical')
    },
    'required_data': {
        'daily_price_data': construct_required_path('data_pull', 'daily_price_data')
    },
...
CalculateTaLibADXMultiParam_params = {
    'params': {'configs': CalculateTaLibADXMultiParam_configs},
    'class': CalculateTaLibADXMultiParam,
    'start_date': RUN_DATE,
    'provided_data': {
        'talib_adx_indicator_data': construct_destination_path('derived_technical')
    },
    'required_data': {
        'daily_price_data': construct_required_path('data_pull', 'daily_price_data')
    },
