...
class QuandlDataCleanup(DataReaderClass):
    ...
    def _resolve_duplicates(self, raw):
        ...
        raw['date'] = (raw['filing_date'].dt.to_period('M') + 1).dt.to_timestamp(
            freq='B'
        )
        raw.drop(
        ...
        raw = raw[
            ['dcm_security_id', 'date', 'filing_date', 'fq']
            + raw.columns.drop(
                ['dcm_security_id', 'date', 'filing_date', 'fq']
            ).tolist()
        ]
        ...
        raw.sort_values(
            ['dcm_security_id', 'filing_date', 'fq', 'dimension'], inplace=True
        )
        raw.drop_duplicates(
            subset=['dcm_security_id', 'filing_date', 'fq'], keep='first', inplace=True
        )
        raw.drop_duplicates(
            subset=['dcm_security_id', 'date'], keep='last', inplace=True
        )
        return raw
    ...
    def _get_close_volume_filing_periods(self, raw, price, dates):
        ...
        filing_periods = (
            raw.groupby(['dcm_security_id', 'dimension'])
            .size()
            .unstack()
            .idxmax(axis=1)
        )
    ...
    def _process_quarterly(self, raw, arq):
        ...
        self.quarterly = (
            quarterly.groupby(['dcm_security_id', 'fq']).last().reset_index()
        )
    ...
    def _process_monthly(self, raw, close, arq, art, months):
        ...
        monthly = pd.concat(
            [arq.stack().reset_index(), art.stack().reset_index()], ignore_index=True
        )
        monthly = pd.merge(monthly, close, how='left', on=['date', 'dcm_security_id'])
        monthly['marketcap'] = (
            monthly['sharesbas'] * monthly['close'] * monthly['sharefactor']
        )
        self.monthly = monthly.sort_values(['dcm_security_id', 'date'])

    def _process_daily(self, raw, close, volume, filing_periods, dates):
        arq_temp = (
            raw[raw['dcm_security_id'].isin(filing_periods['ARQ'])]
            .drop(['dimension', 'date'], axis=1)
        ...
        if "ART" in filing_periods:
            try:
                art = (
                    raw[raw['dcm_security_id'].isin(filing_periods['ART'])]
                ...
                art = (
                    raw[raw['dcm_security_id'] == filing_periods['ART']]
                    .drop('dimension', axis=1)
                    .set_index(['date', 'dcm_security_id'])
                )
        else:
    ...
    def do_step_action(self, **kwargs):
        ...
        dates = pd.date_range(
            raw['date'].min(),
            max(raw['date'].max(), pd.to_datetime(datetime.today())),
            freq='B',
            name='date',
        )
        months = raw['date'].drop_duplicates().sort_values()
        close, volume, filing_periods = self._get_close_volume_filing_periods(
            raw, price, dates
        )

        arq_temp = raw[raw['dcm_security_id'].isin(filing_periods['ARQ'])].drop(
            'dimension', axis=1
        )
        arq = arq_temp.set_index(['date', 'dcm_security_id'])
        dummy_date = pd.Timestamp("2040-12-31")
        ...
        self.data = self.data[
            self.data["dcm_security_id"] < dummy_security_id
        ].reset_index(drop=True)
        self.monthly = self.monthly[
            self.monthly["dcm_security_id"] < dummy_security_id
        ].reset_index(drop=True)
        self.quarterly = self.quarterly[
            self.quarterly["dcm_security_id"] < dummy_security_id
        ].reset_index(drop=True)
        ...
        self.quarterly["dcm_security_id"] = self.quarterly["dcm_security_id"].astype(
            int
        )
        return {
            "quandl_daily": self.data,
            "quandl_monthly": self.monthly,
            "quandl_quarterly": self.quarterly,
        }


############# AIRFLOW PARAMS ##########

QuandlDataCleanup_PARAMS = {
    "params": {},
    "start_date": RUN_DATE,
    'class': QuandlDataCleanup,
    'provided_data': {
        'quandl_daily': construct_destination_path('fundamental_cleanup'),
