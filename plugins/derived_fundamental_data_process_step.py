...
class CalculateDerivedQuandlFeatures(DataReaderClass):
    ...
    def _construct_simple_features(self, sector, daily):
        daily = pd.merge(
            daily,
            sector[['dcm_security_id', 'GICS_Sector', 'GICS_IndustryGroup']],
            how='left',
            on='dcm_security_id',
        )
        ...
        daily['bm_ia_sector'] = daily['bm'] - daily.groupby(['date', 'GICS_Sector'])[
            'bm'
        ].transform('mean')
        daily['bm_ia_indgrp'] = daily['bm'] - daily.groupby(
            ['date', 'GICS_IndustryGroup']
        )['bm'].transform('mean')
    ...
    def _construct_quarterly_pivot_data(self, daily, quarterly):
        ...
        for key in ["assets", "revenue", "capex", "inventory", "shareswa"]:
            df_dict[key] = quarterly.pivot_table(
                index='fq', columns='dcm_security_id', values=key, dropna=False
            ).sort_index()
        ...
        df_dict["delta_capex"] = (
            df_dict["capex"].diff(4)
            / df_dict["assets"].rolling(4, min_periods=1).mean()
        )
        df_dict["delta_inventory"] = (
            df_dict["inventory"].diff(4)
            / df_dict["assets"].rolling(4, min_periods=1).mean()
        )
        ...
        turnover = daily.pivot_table(
            index='date', columns='dcm_security_id', values='turnover', dropna=False
        ).sort_index()
        ...
        for key in ["revenue", "assets"]:
            ...
            df_dict[key + "_growth_qoq"] = (
                df_dict[key + "_growth_qoq"]
                .stack()
                .reset_index()
                .rename(columns={0: key + "_growth_qoq"})
            )
            df_dict["future_" + key + "_growth_qoq"] = (
                df_dict[key].pct_change(1, fill_method=None).shift()
            )
            df_dict["future_" + key + "_growth_qoq"] = (
                df_dict["future_" + key + "_growth_qoq"]
                .stack()
                .reset_index()
                .rename(columns={0: "future_" + key + "_growth_qoq"})
            )
        ...
        daily = daily.rename(
            columns={
                "assets_growth": "asset_growth",
                "assets_growth_qoq": "asset_growth_qoq",
    ...
    def _construct_liquidity_ratio_features(self, price, quandl):
        close = price.pivot_table(
            index='date', columns='dcm_security_id', values='close', dropna=False
        ).sort_index()
        ret = close.pct_change()
        retvol = ret.rolling(21).std()
        maxret = ret.rolling(21).max()
        volume = price.pivot_table(
            index='date', columns='dcm_security_id', values='volume', dropna=False
        ).sort_index()
        dolvol = close * volume
        high = price.pivot_table(
            index='date', columns='dcm_security_id', values='close', dropna=False
        ).sort_index()
        ...
        quandl['sharey'] = (
            quandl['ncfdiv'] + quandl['ncfcommon'] + quandl['ncfdebt']
        ) / quandl['marketcap']
        quandl['shares'] = quandl['sharesbas'] * quandl['sharefactor']
        shares = quandl.pivot_table(
            index='date', columns='dcm_security_id', values='shares', dropna=False
        ).sort_index()
        ...
        df = pd.concat(
            (f.shift(1) for f in objects), keys=features, names=['feature'], axis=1
        ).reset_index()
        df = df.set_index('date').stack().reset_index()
        df.dropna(how='all', subset=features, inplace=True)

        quandl = quandl[
            ['date', 'dcm_security_id', 'ebitda_to_ev', 'ps', 'netmargin', 'sharey']
        ]
    ...
    def do_step_action(self, **kwargs):
        ...
        mapper = (
            kwargs['security_master'][['dcm_security_id', 'ticker']]
            .set_index(['dcm_security_id'])
            .to_dict()['ticker']
        )
        daily['date'] = daily['date'].apply(pd.Timestamp)
        daily = pd.merge(
            daily, extra_features, how="left", on=["date", "dcm_security_id"]
...
CalculateDerivedQuandlFeatures_PARAMS = {
    ...
    'start_date': RUN_DATE,
    'provided_data': {
        'fundamental_features': construct_destination_path('derived_fundamental')
    },
    'required_data': {
        "industry_map": construct_required_path('data_pull', 'industry_mapper'),
        ...
        'quandl_quarterly': construct_required_path(
            'fundamental_cleanup', 'quandl_quarterly'
        ),
