...
class GenerateActiveMatrix(DataReaderClass):
    ...
    def _generate_active_matrix(self, singlename_tickers, data):
        ...
        dates.sort_values()
        indicator = data[data["date"].isin(dates) & data["ticker"].isin(singlename_tickers)][
...
active_matrix_params = {'start_date': "2000-01-01",
                    'end_date': RUN_DATE,
                    'ref_col':'ret_5B'
                }


GenerateActiveMatrixWeekly_params = {'params':active_matrix_params,
 'class':GenerateActiveMatrixWeekly,
                                       'start_date':RUN_DATE,
                                'provided_data': {'active_matrix': construct_destination_path('active_matrix'),
                                                  'active_matrix_weekly': construct_destination_path('active_matrix')},
                                'required_data': {'current_gan_universe':construct_required_path('data_pull','current_gan_universe'),
                                                  'monthly_merged_data_single_names': construct_required_path('filter_dates_single_name','monthly_merged_data_single_names'),
