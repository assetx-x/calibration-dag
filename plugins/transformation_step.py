...
from market_timeline import (
    pick_trading_week_dates,
    pick_trading_month_dates,
    transform_wrapper,
)
...
class TransformEconomicData(DataReaderClass):
    ...
    def _transform_data(
        self, etf_rolling, econ_data, transform_dict, trading_days, date_shift
    ):
        if self.shift_increment == "day":  # Shifts 1-day
            etf_rolling = (
                etf_rolling.shift(date_shift).loc[trading_days].fillna(method="ffill")
            )
            econ_data = (
                econ_data.shift(date_shift).loc[trading_days].fillna(method="ffill")
            )
        else:  # Shifts 1-month
            etf_rolling = (
                etf_rolling.loc[trading_days].fillna(method="ffill").shift(date_shift)
            )
            econ_data = (
                econ_data.loc[trading_days].fillna(method="ffill").shift(date_shift)
            )

        etf_rolling = etf_rolling.apply(
            lambda x: transform_wrapper(x, transform_dict), axis=0
        )
        econ_data = econ_data.apply(
            lambda x: transform_wrapper(x, transform_dict), axis=0
        )
        ...
        econ_data = (
            econ_data[econ_data["date"].isin(date_intersection)]
            .sort_values("date")
    ...
    def do_step_action(self, **kwargs):
        ...
        transform_dict = transform_dict_org.set_index("Unnamed: 0")[
            "transform"
        ].to_dict()
        ...
        max_end_date = min(
            min(etf_rolling.index.max(), econ_data.index.max()), self.end_date
        )
        trading_days = list(
            pick_trading_month_dates(self.start_date, max_end_date, self.mode)
        )
        ...
        self.data = self._transform_data(
            etf_rolling, econ_data, transform_dict, trading_days, date_shift
        )
...
class TransformEconomicDataWeekly(TransformEconomicData):
    ...
    def __init__(
        self, start_date, end_date, monthly_mode, weekly_mode, shift_increment="month"
    ):
        self.weekly_data = None
        self.weekly_mode = weekly_mode
        TransformEconomicData.__init__(
            self, start_date, end_date, monthly_mode, shift_increment
        )
    ...
    def do_step_action(self, **kwargs):
        ...
        monthly_trading_days = list(
            pick_trading_month_dates(self.start_date, max_end_date, self.mode)
        )
        self.data = self._transform_data(
            etf_rolling, econ_data, transform_dict, monthly_trading_days, date_shift
        )

        weekly_trading_days = list(
            pick_trading_week_dates(self.start_date, max_end_date, self.weekly_mode)
        )
        weekly_dated_df = pd.DataFrame({'date': weekly_trading_days})
        self.weekly_data = pd.merge_asof(
            weekly_dated_df, self.data, on='date', direction='backward'
        )

        return {
            'transformed_econ_data': self.data,
            'transformed_econ_data_weekly': self.weekly_data,
        }
...
class CreateIndustryAverage(DataReaderClass):
    ...
    def _average_data(self, mapping, merged_data, averaging_col):
        ...
        data_to_average = pd.merge(
            data_to_average,
            mapping[["dcm_security_id", averaging_col]],
            how="left",
            left_on=["ticker"],
            right_on=["dcm_security_id"],
        )
        ...
        avg_data = (
            data_to_average.groupby(["date", averaging_col])
            .apply(lambda x: x.mean(skipna=True))
            .drop(["ticker", "dcm_security_id"], axis=1)
        )
        for col in avg_data:
    ...
    def do_step_action(self, **kwargs):
        ...
        self.industry_averages = self._average_data(
            industry_mapping, monthly_merged_data, "IndustryGroup"
        )
        self.sector_averages = self._average_data(
            industry_mapping, monthly_merged_data, "Sector"
        )
        return StatusType.Success
...
class CreateIndustryAverageWeekly(CreateIndustryAverage):
    ...
    def do_step_action(self, **kwargs):
        ...
        self.industry_averages_weekly = self._average_data(
            industry_mapping, weekly_merged_data, "IndustryGroup"
        )
        self.sector_averages_weekly = self._average_data(
            industry_mapping, weekly_merged_data, "Sector"
        )
        return {
...
CreateYahooDailyPriceRolling_params = {
    'params': {'rolling_interval': 5},
    'class': CreateYahooDailyPriceRolling,
    'start_date': RUN_DATE,
    'provided_data': {
        'yahoo_daily_price_rolling': construct_destination_path('transformation')
    },
    'required_data': {
        'yahoo_daily_price_data': construct_required_path('data_pull', 'etf_prices'),
    },
}
...
TransformEconomicDataWeekly_params = {
    ...
    'start_date': RUN_DATE,
    'provided_data': {
        'transformed_econ_data': construct_destination_path('transformation'),
        'transformed_econ_data_weekly': construct_destination_path('transformation'),
    },
    'required_data': {
        'yahoo_daily_price_rolling': construct_required_path(
...
CreateIndustryAverageWeekly_params = {
    ...
    'start_date': RUN_DATE,
    'provided_data': {
        'industry_average': construct_destination_path('transformation'),
