...
class FilterMonthlyDatesFullPopulationQuarterly(FilterMonthlyDatesFullPopulation):
    '''
    ...
    PROVIDES_FIELDS = ["monthly_merged_data", "quarterly_merged_data"]
    REQUIRES_FIELDS = ["merged_data"]

    def __init__(self, start_date, end_date, monthly_mode, weekly_mode):
        self.weekly_data = None
        self.weekly_mode = weekly_mode or "BQ"
    ...
    def do_step_action(self, **kwargs):
        ...
        max_end_date = min(self.end_date, df["date"].max())
        chosen_days = list(pick_trading_quarterly_dates(self.start_date, max_end_date, self.weekly_mode))
        filtered_data = df[df["date"].isin(chosen_days)].reset_index(drop=True)
        self.weekly_data = filtered_data

        return {"monthly_merged_data": self.data, "quarterly_merged_data": self.weekly_data}
...
class CreateMonthlyDataSingleNamesQuarterly(CreateMonthlyDataSingleNames):
    PROVIDES_FIELDS = ["monthly_merged_data_single_names", "quarterly_merged_data_single_names"]
    REQUIRES_FIELDS = ["monthly_merged_data", "quarterly_merged_data", "security_master"]
    ...
    def do_step_action(self, **kwargs):
        ...
        weekly_merged_data = kwargs["quarterly_merged_data"]
        ...
        self.weekly_data = weekly_merged_data[weekly_merged_data["ticker"].isin(singlename_tickers)].reset_index(
            drop=True)
        return {'monthly_merged_data_single_names': self.data, 'quarterly_merged_data_single_names': self.weekly_data}
