...
class ConsolidateGANResults(DataReaderClass):
    ...
    def create_gan_target_columns(self, df):
        df["future_return_RF"] = (
            df["future_return_bme"] * df["f_t"]
        )  # TODO: Should be f_t+1 (fix at next full recal)?
        ...
        df[new_col] = df.groupby('date')[col].transform(
    ...
    def do_step_action(self, **kwargs):
        ...
        df = (
            pd.merge(df, gan_factors, how="left", on=["date"])
...
class ConsolidateGANResultsWeekly(ConsolidateGANResults):
    ...
    def do_step_action(self, **kwargs):
        ...
        monthly_df = (
            pd.merge(monthly_df, gan_factors, how="left", on=["date"])
            .set_index(["date", "ticker"])
            .sort_index()
            .reset_index()
        )
        ...
        weekly_df = pd.merge_asof(
            weekly_df, gan_factors, on="date", direction="backward"
        )
        ...
        return {
            'company_data_rf': self.data,
            'company_data_rf_weekly': self.weekly_data,
        }
...
class AddFoldIdToGANResultData(DataReaderClass):
    ...
    def do_step_action(self, **kwargs):
        ...
        df["fold_id"] = pd.cut(
            df["date"], self.cut_dates, labels=pd.np.arange(len(self.cut_dates) - 1)
        )
...
class AddFoldIdToGANResultDataWeekly(AddFoldIdToGANResultData):
    ...
    def do_step_action(self, **kwargs):
        ...
        weekly_df["fold_id"] = pd.cut(
            weekly_df["date"], self.cut_dates, labels=np.arange(len(self.cut_dates) - 1)
        )
        ...
        return {
            'normalized_data_full_population_with_foldId': self.data,
...
ConsolidateGANResultsWeekly_params = {
    ...
    'required_data': {
        'company_data': construct_required_path(
            'save_gan_inputs/save_gan_inputs', 'company_data'
        ),
...
AddFoldIdToGANResultDataWeekly_params = {
    ...
    'required_data': {
        'company_data_rf': construct_required_path(
            'merge_gan_results', 'company_data_rf'
        ),
        'company_data_rf_weekly': construct_required_path(
            'merge_gan_results', 'company_data_rf_weekly'
        ),
    },
}
