    ...
    def do_step_action(self, **kwargs):
        ...
        try:
            ...
            merged_data = merged_data[
                (merged_data["date"] >= self.start_date)
                & (merged_data["date"] <= self.end_date)
            ]
            for i, df in enumerate(
                ...
                merged_data = pd.merge(
                    merged_data, current_df, how="left", on=["date", "ticker"]
                )
            ...
            if self.apply_log_vol:
                for col in [
                    c for c in merged_data.columns if str(c).startswith("volatility_")
                ]:
                    merged_data[col] = np.log(merged_data[col])
            merged_data["log_avg_dollar_volume"] = np.log(
                merged_data["avg_dollar_volume"]
            )
            ...
            merged_data.columns = map(
                lambda x: x.replace("8554", "SPY"), merged_data.columns
            )
            ...
            merged_data["date"] = merged_data["date"].apply(
...
QM_datasets = {
    ...
    'fundamental_features': construct_required_path(
        'derived_fundamental', 'fundamental_features'
    ),
    'target_returns': construct_required_path('targets', 'target_returns'),
    'talib_stochrsi_indicator_data': construct_required_path(
        'derived_technical', 'talib_stochrsi_indicator_data'
    ),
    'volatility_data': construct_required_path('derived_technical', 'volatility_data'),
    'talib_willr_indicator_data': construct_required_path(
        'derived_technical', 'talib_willr_indicator_data'
    ),
    'talib_adx_indicator_data': construct_required_path(
        'derived_technical', 'talib_adx_indicator_data'
    ),
    'talib_ppo_indicator_data': construct_required_path(
        'derived_technical', 'talib_ppo_indicator_data'
    ),
    'beta_data': construct_required_path('derived_simple', 'beta_data'),
    'macd_indicator_data': construct_required_path(
        'derived_simple', 'macd_indicator_data'
    ),
    'correlation_data': construct_required_path('derived_simple', 'correlation_data'),
    'dollar_volume_data': construct_required_path(
        'derived_simple', 'dollar_volume_data'
    ),
    'overnight_return_data': construct_required_path(
        'derived_simple', 'overnight_return_data'
    ),
    'past_return_data': construct_required_path('derived_simple', 'past_return_data'),
    'talib_stoch_indicator_data': construct_required_path(
        'derived_simple', 'talib_stoch_indicator_data'
    ),
    'talib_stochf_indicator_data': construct_required_path(
        'derived_simple', 'talib_stochf_indicator_data'
    ),
    'talib_trix_indicator_data': construct_required_path(
        'derived_simple', 'talib_trix_indicator_data'
    ),
    'talib_ultosc_indicator_data': construct_required_path(
        'derived_simple', 'talib_ultosc_indicator_data'
    ),
    'security_master': construct_required_path('data_pull', 'security_master'),
}

QM_configs = {'apply_log_vol': True, 'start_date': "2000-03-15", 'end_date': RUN_DATE}

QuantamentalMerge_params = {
    'params': QM_configs,
    'class': QuantamentalMerge,
    'start_date': RUN_DATE,
