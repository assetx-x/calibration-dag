...
class MergeAdjustedFutureDataTask(TaskWithStatus):
    def __init__(self, *args, **kwargs):
        super(MergeAdjustedFutureDataTask, self).__init__(*args, **kwargs)
    ...
    def collect_keys(self):
        ...
        if minute_files:
            self._merge_files_into_redshift(minute_files, "future_prices", "manifest", "cob", ["continous_contract"])

        if daily_files:
            self._merge_files_into_redshift(daily_files, "daily_future_prices", "daily_manifest", "date", ["continous_contract"])
