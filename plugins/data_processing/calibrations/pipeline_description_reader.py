...
from calibrations.core import calibration_tasks_registry
...
from etl_workflow_steps import BaseETLStep, BaseETLStage, compose_main_flow_and_engine_for_task, _BASE_FLOW_STEP_RESULT_LABEL
...
class TaskflowPipelineDescriptionBuilder(object):
    ...
    def _order_all_flow_steps_topologically(cls, flow):
        for stage in flow:
            ...
            optimal_ordering = reduce(lambda x,y:x.extend(y) or x, map(list, topological_sorted_steps), [])
    ...
    def build_simplified_flow_file_for_targets(cls, original_flow_file, target_steps, output_file):
        ...
        flow_config[_CALIB_OVERRIDES_KEY] = original_flow_file.get_calib_overrides(_CALIB_OVERRIDES_KEY)
    ...
    @classmethod
    def __collect_steps_for_target(cls, target_step):
        print("\n\n******   Collecting minimalsitic flow to run {0}   ******".format(target_step))
        selected_steps = set()
            ...
            for dataset in required_data:
                ...
                if not possible_providers.intersection(selected_steps) and possible_providers:
                    ...
                    if len(possible_providers)>1:
                        print("\n === There are multiple options to provide data for step {0} as it requires dataset {1}. "
                              "Please select one of the following".format(step, dataset))
    ...
    @classmethod
    def build_flow_from_scratch(cls, target_step, output_file):
        if target_step not in CalibrationTaskRegistry.registered_classes:
            if target_step in CalibrationTaskRegistry.skipped_classes:
                raise ValueError("TaskflowPipelineDescriptionBuilder.build_flow_from_scratch - the target step {0} is a"
                                 " class that could not be registered on CalibrationTaskRegistry; verify if it contains"
                                 " abstract methods".format(target_step))
            else:
                raise ValueError("TaskflowPipelineDescriptionBuilder.build_flow_from_scratch - the target step {0} is"
                                 " not a recognized class name for CalibrationTaskflowTask".format(target_step))
        flow_steps = cls.__collect_steps_for_target(target_step)
        proposed_flow = {"{0}_flow".format(target_step): flow_steps}
...
class TaskflowPipelineDescriptionValidation(object):
    ...
    def _verify_basic_format(cls, pipeline_data, pipeline_file_path):
        ...
        for k in expected_section_labels:
            if k not in pipeline_data:
                raise KeyError("TaskflowPipelineDescriptionValidation.__verify_basic_format - the file {0} does not contains a "
    ...
    def _verify_flow_section_format(cls, pipeline_data, pipeline_file_path):
        ...
        wrong_step_structure = ["{0}.{1}".format(k, l) for l in flow[k] for k in flow
        ...
        wrong_steps = ["{0}.{1}".format(k, l) for l in flow[k] for k in flow
    ...
    def _verify_flow_integrity(cls, pipeline_data, pipeline_file_path):
        ...
        flow_inputs = [getattr(k, "REQUIRES_FIELDS", []) for k in flow_classes]
        flow_inputs = set(reduce(lambda x,y:x.extend(y) or x, flow_inputs, []))
        flow_outputs = [getattr(k, "PROVIDES_FIELDS", []) for k in flow_classes]
        flow_outputs = set(reduce(lambda x,y:x.extend(y) or x, flow_outputs, []))
        flow_outputs = flow_outputs.union(BaseETLStep.REQUIRES_FIELDS)
        ...
        if missing_inputs:
            ...
            for k in missing_inputs:
                tasks_with_missing_input = [l.__name__ for l in flow_classes if k in getattr(l, "REQUIRES_FIELDS", [])]
    ...
    def __verifiy_snapshot_and_caching_config(cls, pipeline_data, pipeline_file_path):
        ...
        if "*" in existing_cache:
            known_caches = map(lambda x:os.path.basename(x).replace(".h5", ""),
                                  glob(os.path.join(snapshot_location, "*.h5")))
            known_caches = [k for k in known_caches if k in flow.keys()]
...
class TaskflowPipelineDescription(object):
    ...
    def get_configuration_of_step(self, step_name):
        return self.step_configurations[step_name]
...
class TaskflowPipelineRunner(object):
    ...
    def __create_taskflow_stage(self, stage_name):
        ...
        for step_name in self.pipeline_description.steps_iterator_for_stage(stage_name):
            ...
            step_config = json.loads(pyhocon.tool.HOCONConverter.to_json(step_config))
    ...
    def __run_pipeline_stage(self, stage_name, flow_parameters, dependency_data, create_snapshot_for_stage=True):
        ...
        if create_snapshot_for_stage:
            ...
            snapshot_taskflow_engine_results(engine, snapshot_location)
    ...
    def run(self, flow_parameters):
        if flow_parameters.run_mode is TaskflowPipelineRunMode.Calibration:
            self.pipeline_description.enable_calibration_overrides()
        elif flow_parameters.run_mode is TaskflowPipelineRunMode.Historical:
            self.pipeline_description.disable_overrides()
        elif flow_parameters.run_mode is TaskflowPipelineRunMode.Test:
            self.pipeline_description.enable_test_overrides()
        else:
            raise RuntimeError("TaskflowPipelineRunner.run - unknown runmode {0}"
        ...
        try:
            for stage in self.pipeline_description:
                ...
                if (self.pipeline_description.can_reuse_cache(stage) and os.path.exists(snapshot_loc)):
                    stage_results = read_taskflow_engine_snapshot(snapshot_loc)
                else:
                    stage_results = self.__run_pipeline_stage(stage, flow_parameters, flow_results,
                ...
                for k in BaseETLStep.REQUIRES_FIELDS:
        ...
        return all_step_results, flow_results
...
if __name__=="__main__":
    ...
    #base_file = r"C:\DCM\dcm-intuition\processes\data_processing\calibrations\views_in_market_trading\market_view_calibration.conf"
    #base_file = r"C:\DCM\dcm-intuition\processes\data_processing\calibrations\basket_pair_trading\basket_pairs_calibration.conf"
    #base_file = r"C:\DCM\dcm-intuition\processes\data_processing\calibrations\fundamental_rebalancing_trader\fundamental_rebalancing_calibration.conf"
    #base_file = r"C:\DCM\dcm-intuition\processes\data_processing\calibrations\jump_recovery_trading\jump_recovery_calibration.conf"
    #base_file = r"C:\DCM\dcm-intuition\processes\data_processing\calibrations\experimental\news_momentum_experiment.conf"
    base_file = r"C:\DCM\dcm-intuition\processes\data_processing\calibrations\quantamental_ml\quantamental_with_econ_full.conf"

    #flow_file = r"C:\DCM\dcm-intuition\processes\data_processing\calibrations\views_in_market_trading\market_view_calibration_full.conf"
    #flow_file = r"C:\DCM\dcm-intuition\processes\data_processing\calibrations\basket_pair_trading\basket_pairs_calibration_simplified.conf"
    #flow_file = r"C:\DCM\dcm-intuition\processes\data_processing\calibrations\fundamental_rebalancing_trader\fundamental_rebalancing_calibration_full.conf"
    #flow_file = r"C:\DCM\dcm-intuition\processes\data_processing\calibrations\jump_recovery_trading\jump_recovery_calibration_full.conf"
    #flow_file = r"C:\DCM\dcm-intuition\processes\data_processing\calibrations\views_in_market_trading\calibrationdates_test.conf"
    #flow_file = r"C:\DCM\dcm-intuition\processes\data_processing\calibrations\fundamental_rebalancing_trader\create_intervals_test.conf"

    #TaskflowPipelineDescriptionBuilder.build_simplified_flow_file_for_targets(base_file, ["GenerateKMeansClusters"], flow_file)
    #TaskflowPipelineDescriptionBuilder.build_flow_from_scratch("GenerateKMeansClusters", flow_file)

    #TaskflowPipelineDescriptionBuilder.build_simplified_flow_file_for_targets(base_file, ["CalibrationDatesFundamental"],
    #                                                                          flow_file)

    from calibrations.common.taskflow_params import BasicTaskParameters, TaskflowPipelineRunMode
    # Historical
    #flow_parameters = BasicTaskParameters(pd.Timestamp("2018-12-21"), pd.Timestamp("2015-09-01"), pd.Timestamp("2018-01-13"),
                                          #pd.Timestamp("1701-09-03"), False)
    # Calibration BP Historical
    #target_dt = pd.Timestamp("2019-01-04")
    #flow_parameters = BasicTaskParameters(pd.Timestamp("2018-12-31"), pd.Timestamp("2016-06-01"),
                                          #pd.Timestamp("2018-12-31"), pd.Timestamp("1701-09-03"), TaskflowPipelineRunMode.Historical)
    # Calibration BP Live
    #flow_parameters = BasicTaskParameters(pd.Timestamp("2018-12-31"), target_dt - pd.tseries.frequencies.to_offset('400B'),
    #                                      target_dt, pd.Timestamp("1701-09-03"), True)

    # Calibration MV Historical
    ...
    # Live
    target_dt = pd.Timestamp("2019-12-01")
    start_dt = pd.Timestamp("2000-01-03")
    #start_dt = pd.Timestamp("2007-01-02")
    #start_dt = pd.Timestamp("1997-01-02")
    flow_parameters = BasicTaskParameters(target_dt, start_dt, target_dt, target_dt, TaskflowPipelineRunMode.Historical)

    #TaskflowPipelineDescription.build_valid_pipeline_desc_file_from_flow(base_file, flow_file)
    pipeline_desc = TaskflowPipelineDescription(base_file)
    #pipeline_desc = TaskflowPipelineDescription(flow_file)
