...
flow_file = os.path.join(os.path.dirname(__file__), "..", "quantamental_ml.conf")
results_dir = os.path.join(os.path.dirname(__file__.replace("dcm-intuition", "test_data")), "calibration_data")

@pytest.mark.skip()
def test_quantamental_ml(step_class_name, test_flow_parameters, precalculated_pipeline_results, pipeline_to_test,
...
if __name__=="__main__":
    ...
    test_quant_ml = pytest.create_calibration_tests(test_quantamental_ml, flow_file, results_dir, pytest.filter_data_reader_steps,
                                                    None, None)
