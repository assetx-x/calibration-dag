...
def get_config():
    ...
    if not __config:
        ...
        data = {
            ...
            econ_cols = [COMPAPFFx, EWG_close, IPDCONGD, CUSR0000SA0L5, WPSID61, CUSR0000SA0L2, HWI, PPO_21_126_Industrials, CPIULFSL,
                         ...
                         PPO_21_126_ConsumerDiscretionary, PPO_21_126_InformationTechnology, VXOCLSx, CUSR0000SAC,
                         bm_ConsumerDiscretionary, divyield_ConsumerStaples, SPY_close, DNDGRG3M086SBEA, CP3Mx, EWJ_volume, T5YFFM,
                         GS10, USTRADE, UEMP5TO14, EXUSUKx, PPO_21_126_Utilities, ACOGNO, divyield_InformationTechnology, T10YFFM,
                         bm_Utilities, CUMFNS, CES0600000008, FEDFUNDS, GS1, GS5, T1YFFM, AAAFFM, EXJPUSx, OILPRICEx, SPY_volume,
...
def build_timeseries_modified(econ_data, company_data, return_data, per_moment_time_weights, per_company_time_weights, average_weights, time_steps):
...
def get_data():
    ...
    df = pd.read_hdf(os.path.join(data_dir, "company_data.h5")).fillna(0.0)
    company_data = df.set_index(["date", "ticker"]).sort_index().to_panel().transpose(1, 2, 0).fillna(0.0)
    econ_data = pd.read_hdf(os.path.join(data_dir, "econ_data_final.h5"))
    econ_data = econ_data.set_index(["date"]).sort_index()
    active_matrix = pd.read_hdf(os.path.join(data_dir, "active_matrix.h5")).fillna(0)
    return_data = pd.read_hdf(os.path.join(data_dir, "future_returns.h5")).fillna(0.0)
...
class DataGeneratorMultiBatchFast(object):
    def __init__(self, data, x_cols, econ_cols, y_col, batch_size, time_steps, data_splits=None, data_split_integer_override=None):
    ...
    def build_train_data(self):
        ...
        build_timeseries_modified(macro_factor, characteristics, returns, per_moment_time_weights, per_company_time_weights,
    ...
    def generate_train_data(self):
        ...
        while True:
            ...
            inputs = [econ[start_index:end_index], comp[start_index:end_index], rets[start_index:end_index],
                      mom_weights[start_index:end_index], comp_weights[start_index:end_index], avg_weights[start_index:end_index],
...
class AssetPricingGAN(object):
    def __init__(self, n_macro_features=1, n_company_fetures=2, n_assets=1, time_steps=250, batch_size=10, optimizer="adam",
        ...
        self.gan_model = Model([self.macro_input_layer, self.company_input_layer, self.return_sequences,
                                self.moment_weight_input_layer, self.company_weight_input_layer, self.overall_loss_weight_input_layer,
    ...
    def create_loss(self):
        ...
        tiled_discount_factor = Lambda(lambda x:tile(k_expand_dims(x), (1,1,) + tuple(self.conditional_network.output.shape[2:])),
        ...
        time_averaged_loss =  Lambda(lambda x:k_sum(x, axis=1), name="time_averaged_losses_per_company_moment")(moment_factors)
        tiled_moment_weights = Lambda(lambda x:tile(k_expand_dims(x), (1, 1, time_averaged_loss.shape[-1])), name="tiled_moment_weights")(self.moment_weight_input_layer)
    ...
    def create_input_layers(self):
        ...
        self.company_input_layer = Input(shape=(self.time_steps, self.n_assets, self.n_company_fetures), name="company_data")
        ...
        self.flattened_company_inputs = Reshape((self.time_steps or -1, self.n_assets*self.n_company_fetures,), name="flat_company_data")(self.company_input_layer)
    ...
    def __build_network(self, cfg_section_name):
        ...
        for k in range(cfg_section["n_rnn_hidden_layers"]):
            lstm_layer = LSTM(**cfg_section["rnn_hidden_layer"], name="{0}-rnn-{1}".format(cfg_section_name, k))(lstm_layer)
        ffn_input = concatenate([lstm_layer, self.flattened_company_inputs], name="{0}-ffn-input".format(cfg_section_name))
        ffn = ffn_input
        for k in range(cfg_section["n_ffn_hidden_layers"]):
            ffn = TimeDistributed(Dense(**cfg_section["ffn_hidden_layer"]), name="{0}-fnn-{1}".format(cfg_section_name, k+1))(ffn)
            if cfg_section.get("ffn_droput_layer", None) and cfg_section["ffn_droput_layer"]["rate"]:
                ffn = TimeDistributed(Dropout(**cfg_section["ffn_droput_layer"]), name="{0}-dropout-{1}".format(cfg_section_name, k+1))(ffn)
        output_multiplier = cfg_section["output_layer_units_multiplier"]
        ffn_output_raw = TimeDistributed(Dense(self.n_assets*output_multiplier), name="{0}-output-raw".format(cfg_section_name))(ffn)
        ffn_output = Reshape((self.time_steps, self.n_assets, output_multiplier), name="{0}-output".format(cfg_section_name))(ffn_output_raw)
    ...
    def train_network(self, data_generator, epochs=1, epochs_discriminator=None, gan_iterations=1, verbose=1,
        ...
        for gan_iter in range(gan_iterations):
            for training_mode in [TrainingMode.sdf, TrainingMode.conditional]:
                ...
                if run_validation:
                    self.history = self.gan_model.fit_generator(data_generator.generate_train_data(), steps_per_epoch, epoch_counter+current_epochs, verbose,
                                                                validation_data = data_generator.generate_validation_data(),
                                                                validation_steps=steps_per_epoch, validation_freq=1,
                                                                callbacks=cbs, initial_epoch=epoch_counter, shuffle=shuffle)
                else:
                    self.history = self.gan_model.fit_generator(data_generator.generate_train_data(), steps_per_epoch, epoch_counter+current_epochs, verbose,
                                                                callbacks=cbs, initial_epoch=epoch_counter, shuffle=shuffle)
    ...
    def train_network_stateful(self, data_generator, epochs=1, epochs_discriminator=None, gan_iterations=1, verbose=1, run_validation=True,
                               shuffle=False):
        epoch_counter = 0
        cbs = self.prepare_training_callbacks(epochs)
        ...
        for gan_iter in range(gan_iterations):
            for training_mode in [TrainingMode.sdf, TrainingMode.conditional]:
                ...
                for i in range(current_epochs):
                    ...
                    if run_validation:
                        self.history = self.gan_model.fit_generator(data_generator.generate_train_data(), steps_per_epoch, epoch_counter+1, verbose,
                                                                    validation_data=data_generator.generate_validation_data(),
                                                                    validation_steps=validation_steps_per_epoch, validation_freq=1,
                                                                    callbacks=cbs, initial_epoch=epoch_counter, shuffle=shuffle)
                    else:
                        self.history = self.gan_model.fit_generator(data_generator.generate_train_data(), steps_per_epoch, epoch_counter+1, verbose,
                                                                    callbacks=cbs, initial_epoch=epoch_counter, shuffle=shuffle)
...
class ModelTraining(object):
    ...
    def __init__(self, data_dir, insample_cut_date, lr=0.0001, decay=0.0, epsilon=1e-8, epochs=150, epochs_discriminator=50, gan_iterations=3):
        self.data_dir = data_dir
        self.weight_path = os.path.join(data_dir, "saved_weights.h5")
        ...
        n_obs = len(active_matrix)-time_steps #TODO
        ...
        self.gan_network = AssetPricingGAN(n_macro_features, n_company_features, n_assets, time_steps=time_steps,
                                           batch_size=batch_size, training_loss=linear_pred_loss,
                                           optimizer=Adam(lr=lr, decay=decay, epsilon=epsilon))
    
    def _load_data(self):
        return load_data(self.data_dir)
    
    def train_model(self):
        self.gan_network.train_network_stateful(self.data_generator, self.epochs, self.epochs_discriminator, self.gan_iterations, 1,
                                                False, False)
    
    def save_model(self):
        self.gan_network.gan_model.save_weights(self.weight_path)
...
class SDFExtraction(object): # This is a complete version with data and gan network
    def __init__(self, data_dir, insample_cut_date):
        ...
        self.data_generator = DataGeneratorMultiBatchFast((company_data, econ_data, return_data, active_matrix),
                                                          x_cols, econ_cols, y_col, batch_size, time_steps,
                                                          data_splits, data_split_integer_override)
        gan_network = AssetPricingGAN(n_macro_features, n_company_features, n_assets, time_steps=time_steps, batch_size=batch_size,
                                      training_loss=linear_pred_loss, optimizer=Adam(lr=0.00001, decay=0.0, epsilon=1e-8))
        ...
        self.intermediate_model = Model(inputs=self.sdf_network.input, outputs=self.sdf_network.get_layer("sdf-rnn-0").output)
...
def train_gan_model(data_dir, insample_cut_date, epochs, epochs_discriminator, gan_iterations):
    model = ModelTraining(data_dir, insample_cut_date, lr=0.0001, decay=0.0, epsilon=1e-8, epochs=epochs,
                          epochs_discriminator=epochs_discriminator, gan_iterations=gan_iterations)
    model.train_model()
    model.save_model()
...
if __name__=="__main__":
    if len(sys.argv)!=7:
        print("Missing arguments: requires [data_dir] [insample_cut_date] [epochs] [epochs_discriminator] [gan_iterations] [mode]")
        raise
    else:
        data_dir = sys.argv[1]
        data_dir = Path(data_dir)
        insample_cut_date = pd.Timestamp(sys.argv[2])
        epochs = int(sys.argv[3])
        epochs_discriminator = int(sys.argv[4])
        gan_iterations = int(sys.argv[5])
        mode = int(sys.argv[6])
