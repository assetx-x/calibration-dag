...
class SQLReaderDailyIndexData(SQLReader):
    ...
    @classmethod
    def get_default_config(cls):
        return {"table_name": "daily_index", "index_names": ["SPX", "PPUT", "CMBO", "BFLY", "RXM", "VIX", "VXST", "VXMT", "VIX3M"]}
...
class SQLReaderRavenpackMomentumIndicator(SQLReader):
    ...
    def compose_query(self, base_query, **kwargs):
        date_st = self.task_params.start_dt.tz_convert("UTC") if self.task_params.start_dt.tzinfo else self.task_params.start_dt
        date_end = self.task_params.end_dt.tz_convert("UTC") if self.task_params.end_dt.tzinfo else self.task_params.end_dt
...
class SQLSecurityMasterReader(SQLReader):
    ...
    def _post_process_pulled_data(self, data, **kwargs):
        ...
        if self.task_params.run_mode==TaskflowPipelineRunMode.Test:
            additional_query = "select ticker from whitelist where run_dt in ('{0}') and as_of_end is null".format(self.task_params.universe_dt)
        ...
        for ticker in self.sector_etfs:
            try:
                ...
                df = web.get_data_yahoo(ticker, self.start_date,self.end_date
                                        ).fillna(method="ffill")[["Adj Close","Volume"]].rename(columns={"Adj Close": close_col,
                                                                                                         "Volume": vol_col})
