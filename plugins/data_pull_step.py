...
class S3IndustryMappingReader(GCPReader):
    ...
    def _post_process_pulled_data(self, data, **kwargs):
        ...
        data["dcm_security_id"] = data["dcm_security_id"].astype(int)
        data = data[
            data["dcm_security_id"].isin(list(sec_master["dcm_security_id"]))
...
class S3EconTransformationReader(GCPReader):
    ...
    def _pull_data(self, **kwargs):
        ...
        df = pd.DataFrame(
            list(transform_dict.values()), index=list(transform_dict.keys())
        ).reset_index()
...
class YahooDailyPriceReader(DataReaderClass):
    ...
    def _prepare_to_pull_data(self, **kwargs):
        self.start_date = (
            self.start_date
        )  # if pd.notnull(self.start_date) else self.task_params.start_dt
        self.end_date = (
            self.end_date
        )  # if pd.notnull(self.end_date) else self.task_params.end_dt

    def _get_data_lineage(self):
        pass

    def _pull_data(self, **kwargs):
        etf_prices = download_yahoo_data(
            self.sector_etfs, self.start_date, self.end_date
        )
        return etf_prices
...
class S3RussellComponentReader(DataReaderClass):
    ...
    def do_step_action(self, **kwargs):
        ...
        r3k[['wt-r1', 'wt-r1g', 'wt-r1v']] = r3k[['wt-r1', 'wt-r1g', 'wt-r1v']].fillna(
            0
        )
        ...
        df = df[
            df["dcm_security_id"].isin(list(sec_master["dcm_security_id"]))
        ].reset_index(drop=True)
...
class SQLMinuteToDailyEquityPrices(GCPReader):
    ...
    def _post_process_pulled_data(self, price, **kwargs):
        ...
        price = (
            ...
            .mean()[['open', 'high', 'low', 'close', 'volume', 'dcm_security_id']]
            .reset_index()
            .sort_values(by=['date'])
        )
...
class CalibrationDates(DataReaderClass):
    ...
    def __init__(
        ...
        if not isinstance(read_csv_kwargs, dict):
            raise TypeError(
                "CalibrationDates.__init__ - read_csv_kwargs must be a dictionary; "
                "received a {0}".format(type(read_csv_kwargs))
            )
        if cache_file:
            cache_file = (
                os.path.abspath(os.path.join(os.path.dirname(__file__), cache_file))
                if not os.path.isabs(cache_file)
                else cache_file
            )
            if not os.path.exists(cache_file):
                raise RuntimeError(
                    "CalibrationDates.__init__ - file {0} does not exists".format(
                        cache_file
                    )
                )
    ...
    def get_default_config(cls):
        cache_file = {
            "cache_file": "./configuration/interevals_as_of_20181218.csv",
    ...
    def post_process_intervals(self, intervals):
        raise NotImplementedError(
            "CalibrationDates.post_process_intervals must be overriden by derived class"
        )

    @abstractmethod
    def calculate_intervals(self):
        raise NotImplementedError(
            "CalibrationDates.calculate_intervals must be overriden by derived class"
        )

    @abstractmethod
    def calculate_intervals_for_calibration(self):
        raise NotImplementedError(
            "CalibrationDates.calculate_intervals_for_calibration must be overriden by "
            "derived class"
        )
...
class CalibrationDatesJump(CalibrationDates):
    ...
    def __init__(
        ...
        cache_file=None,
        target_dt=None,
        intervals_start_dt=None,
        intervals_end_dt=None,
        ...
        intervals_start_dt = (
            pd.Timestamp(intervals_start_dt) if intervals_start_dt else None
        )
        ...
        CalibrationDates.__init__(
            self, cache_file, read_csv_kwargs, force_recalculation, save_intervals_to
        )
    ...
    def cache_path(self):
        ...
        _cache_path = os.path.join(
            project_root, 'plugins', 'data_processing', 'intervals_for_jump.csv'
        )
        return _cache_path

    def calculate_intervals(self):
        self.intervals_start_dt = (
            self.intervals_start_dt
        )  # or self.task_params.start_dt.normalize()
        self.intervals_end_dt = (
            self.intervals_end_dt
        )  # or self.task_params.end_dt.normalize()
        entry_dates = list(
            marketTimeline.get_trading_days(
                self.intervals_start_dt, self.intervals_end_dt
            ).normalize()
        )
        exit_dates = map(
            lambda x: marketTimeline.get_trading_day_using_offset(
                x, self.holding_period_in_trading_days
            ),
            entry_dates,
        )
        intervals = pd.concat(
            [pd.Series(entry_dates), pd.Series(exit_dates)],
            axis=1,
            keys=["entry_date", "exit_date"],
        )
        quarter_func = lambda x: str(
            x
            - pd.tseries.frequencies.QuarterEnd(normalize=True)
            + pd.tseries.frequencies.to_offset('1D')
        ).split(" ")[0]
...
class SQLMinuteToDailyEquityPrices_2_0_edit(DataReaderClass):
    ...
    def do_step_action(self, **kwargs):
        ...
        """.format(
            self.start_date, self.end_date, tuple(security_master['ticker'].values)
        )
