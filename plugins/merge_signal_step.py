...
class QuantamentalMergeSignals(DataReaderClass):
    ...
    def do_step_action(self, **kwargs):
        ...
        merged_data = pd.merge(
            all_data,
            all_signal_ranks.drop(self.drop_column, axis=1),
            how="left",
            on=["date", "ticker"],
        )
...
class QuantamentalMergeSignalsWeekly(QuantamentalMergeSignals):
    ...
    def do_step_action(self, **kwargs):
        ...
        all_signal_ranks_weekly = kwargs[self.__class__.REQUIRES_FIELDS[3]].copy(
            deep=True
        )
        self.data = pd.merge(
            all_data,
        ...
        self.weekly_data = pd.merge(
            all_data_weekly,
            all_signal_ranks_weekly.drop(self.drop_column, axis=1),
            how="left",
            on=["date", "ticker"],
        )

        return {
            'data_full_population_signal': self.data,
            'data_full_population_signal_weekly': self.weekly_data,
        }
...
params_dict = {'drop_column': "future_ret_21B"}


QuantamentalMergeSignalsWeekly_params = {
    'params': params_dict,
    'class': QuantamentalMergeSignalsWeekly,
    'start_date': RUN_DATE,
    'provided_data': {
        'data_full_population_signal': construct_destination_path('merge_signal'),
    ...
    'required_data': {
        ...
        'intermediate_signals': construct_required_path(
            'intermediate_model_training', 'intermediate_signals'
        ),
        'intermediate_signals_weekly': construct_required_path(
            'intermediate_model_training', 'intermediate_signals_weekly'
        ),
    },
}
